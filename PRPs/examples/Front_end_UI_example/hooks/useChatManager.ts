...
import { generateContentWithFiles } from '../services/geminiService';
...
export const useChatManager = () => {
  ...
  const sendMessage = async (text: string, files: File[]) => {
    ...
    try {
        const modelResponseText = await generateContentWithFiles(text, files);
        ...
        if(activeSession.messages.length === 0) { // First message sets title
