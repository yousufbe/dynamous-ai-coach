# Implementation Plan: RAG Retrieval & LLM Integration Focus

## Overview
This plan narrows the previously defined end-to-end work onto the highest-impact
next steps: wiring retrieval and LLM generation into the agent, tightening the
frontend/backend contract, and strengthening validation plus observability.
Frontend transport switching to the local FastAPI backend has been implemented,
so the focus now shifts to returning grounded answers with citations and making
those behaviours testable and debuggable.

The plan therefore emphasizes:

- Implementing typed retrieval services over the existing Docling + PGVector
  schema and exposing citation-ready chunk metadata.
- Introducing a configurable, testable LLM client abstraction and integrating it
  into `RAGAgent.chat`.
- Extending backend tests and documentation so `/chat` can be validated in a
  repeatable, environment-agnostic way.
- Keeping Archon tasks and plan documents synchronized with the actual state of
  the codebase.
